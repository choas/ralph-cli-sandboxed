...
export interface CliProviderConfig {
  ...
  instructionFile?: string; // Provider-specific instruction file (e.g., "CLAUDE.md", "GEMINI.md", "AGENTS.md")
  modelConfig?: {
    envVar?: string;
    note?: string;
  };
...
export function generatePrompt(
  ...
  return resolvePromptVariables(template, {
    language:
      Object.keys(LANGUAGES).find((k) => LANGUAGES[k].name === config.name) ||
      "none",
    checkCommand: config.checkCommand,
    testCommand: config.testCommand,
    technologies,
...
// Provider-specific extra instructions (appended to instruction files)
const PROVIDER_EXTRA_INSTRUCTIONS: Record<string, string> = {
  gemini: GEMINI_MD,
};

// Generate instruction file content from skills for a given CLI provider
export function generateInstructionFileContent(
  skills: SkillDefinition[],
  providerKey?: string,
): string {
  const sections: string[] = [];

  sections.push("# Ralph CLI — AI Agent Instructions\n");
  sections.push(
    "This file is auto-generated by Ralph CLI. Do not edit manually.\n",
  );
  ...
  // Append provider-specific extra instructions
  if (providerKey && PROVIDER_EXTRA_INSTRUCTIONS[providerKey]) {
    sections.push(PROVIDER_EXTRA_INSTRUCTIONS[providerKey]);
  }
